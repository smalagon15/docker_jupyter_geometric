...
class Neo4jDriver:
    ...
    def load_csv(self, filename="file:///Blue and fin whales Southern California 2014-2015 - Argos data.csv"):
        with self.driver.session() as session:
            result = session.read_transaction(self._load_csv, filename)
            for record in result:
                print("Loaded sighting: {record}".format(record=record))
